    ...
    for (;;) {
        ...
        if (m_queue.is_empty()) {
            ...
            for (u64 i = 0; i < entry.piece_count; i++) {
                auto result = entry.data_file_map->check_piece(i, i == entry.piece_count - 1);

                if (result.is_error()) {
                    dbgln("Checker: error while checking piece {}: {}", i, result.error());
                    retvalue = Error::copy(result.error());
                    break;
                }
                bitfield.set(i, result.release_value());
