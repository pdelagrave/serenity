...
ErrorOr<void> Announcer::announce(EventType event_type)
{
    ...
    url.set_query(TRY(String::formatted(
                          ...
                          event_type_param.to_string().release_value()))
    ...
    request->on_buffered_request_finish = [&, request, event_type](bool success, auto total_size, auto&, auto status_code, ReadonlyBytes payload) {
        ...
        auto err = [&]() -> ErrorOr<void> {
            ...
            if (response.contains("failure reason")) {
                ...
                dbgln("Tracker returned failure:  {}", response.get_string("failure reason"));
                return {};
            }
            ...
            if (response.has<List>("peers")) {
                for (auto peer : response.get<List>("peers")) {
                    ...
                    auto ip_address = IPv4Address::from_string(peer_dict.get_string("ip").view());
