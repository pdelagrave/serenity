...
ErrorOr<NonnullRefPtr<PeerConnection>> PeerConnection::try_create(NonnullOwnPtr<Core::TCPSocket>& socket, NonnullRefPtr<Core::Notifier> write_notifier, size_t input_buffer_size, size_t output_buffer_size)
{
    ...
    return adopt_nonnull_ref_or_enomem(new (nothrow) PeerConnection(socket, write_notifier, input_buffer, output_buffer));
}

PeerConnection::PeerConnection(NonnullOwnPtr<Core::TCPSocket>& socket, NonnullRefPtr<Core::Notifier>& write_notifier, CircularBuffer& input_message_buffer, CircularBuffer& output_message_buffer)
    : socket(move(socket))
